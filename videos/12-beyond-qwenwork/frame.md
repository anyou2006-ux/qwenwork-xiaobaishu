...
description: >
  ...
  are identical and sacred — the green/pink/cream editorial triad, LXGW WenKai at weight 500
  (optical-size axis) for all display + LXGW WenKai 500 uppercase chrome, flat paper depth (no
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "LXGW WenKai", cqw: 1.56, weight: 400, lineHeight: 1.38 }
  body-card:{ fontFamily: "LXGW WenKai", cqw: 1.35, weight: 400, lineHeight: 1.34 }
  label:   { fontFamily: "LXGW WenKai", px: 26, cqw: 1.35, weight: 500, tracking: "0.18em", upper: true }
  caption-mono:{ fontFamily: "LXGW WenKai", px: 24, cqw: 1.25, weight: 500, tracking: "0.14em", upper: true }
  # — display ramp (LXGW WenKai weight 500, opsz, negative tracking) —
  title-card-sm:{ fontFamily: "LXGW WenKai", cqw: 2.9, weight: 500, lineHeight: 0.98, tracking: "-0.01em" }
  title-card:{ fontFamily: "LXGW WenKai", cqw: 3.5, weight: 500, lineHeight: 0.96, tracking: "-0.01em" }
  headline:{ fontFamily: "LXGW WenKai", cqw: 4.4, weight: 500, lineHeight: 1.0, tracking: "-0.02em" }
  headline-xl:{ fontFamily: "LXGW WenKai", cqw: 5.0, weight: 500, lineHeight: 0.96, tracking: "-0.02em" }
  display:{ fontFamily: "LXGW WenKai", cqw: 7.3, weight: 500, lineHeight: 1.02, tracking: "-0.02em" }
  display-hero:{ fontFamily: "LXGW WenKai", cqw: 11.5, weight: 500, lineHeight: 0.92, tracking: "-0.02em" }
  stat-figure:{ fontFamily: "LXGW WenKai", cqw: 11.5, weight: 500, lineHeight: 0.92, tracking: "-0.03em" }
  stat-figure-unit:{ fontFamily: "LXGW WenKai", cqw: 5.7, weight: 500, lineHeight: 0.92 }
  name:    { fontFamily: "LXGW WenKai", cqw: 2.3, weight: 600, lineHeight: 1.0 }
...
components:
  topbar:
    typography: "{typography.label} (LXGW WenKai) + monogram-circle or counter"
...
Penguin classic or a quiet annual report. One confident voice, **LXGW WenKai at weight 500**
(optical-size axis engaged), carries every headline and stat up to ~12cqw; **LXGW WenKai** at
...
- **LXGW WenKai weight 500** (opsz, negative tracking) for all display; **serif 400** body; **LXGW WenKai 500** uppercase chrome.
...
- **Squint** — one LXGW WenKai moment dominates at 3–6× its neighbor; the surface block centers the eye.
...
Two ramps. The **reading/chrome ramp** (LXGW WenKai **400** body 1.56cqw; LXGW WenKai labels
in px) carries copy + chrome; the **display ramp** (LXGW WenKai **500** `title-card-sm` 2.9cqw →
...
**Ground** `{colors.green}`, `slide-pad`. **Composes** topbar (mono label + monogram-circle), display-hero, body lede, footline. **Focal** a 2-line LXGW WenKai `display-hero` in pink (one word in cream), left. **Chrome** mono topbar; mono footline. **Accent** pink display + the monogram circle. **Silence** ~50%. **Fixed** serif 500, flat, monogram 2px pink. **Free** title, which word is cream. **Density** low.
...
**Ground** `{colors.green}`, `slide-pad`. **Composes** topbar, kpi-block (mono tag + 220px stat-figure + serif description), footline. **Focal** a LXGW WenKai `stat-figure` (~11.5cqw, +unit) in pink over a 2px pink rule. **Chrome** mono topbar + footline. **Accent** the pink figure. **Silence** ~55%. **Fixed** serif 500 figure, 2px accent rule, flat. **Free** the figure (from script), tag, description. **Density** low.

### 4 · Statement (quote · move: display serif · cream · left)

**Ground** `{colors.cream}`, generous pad. **Composes** topbar, display, name (600), caption-mono role. **Focal** a 2-line LXGW WenKai `display` (~7.3cqw) in green; a 600-weight attribution name + mono role beneath. **Chrome** mono topbar. **Accent** none — the serif carries it. **Silence** ~55%. **Fixed** serif 500 display + 600 name only, no italic. **Free** quote, name, role. **Density** low.
...
- Run every display in **LXGW WenKai weight 500**, opsz, negative-tracked, tight line-height; **body at 400**.
- Set every label/caption/axis/footline in **LXGW WenKai 500 uppercase, 0.08–0.18em**.
...
- **Type** — LXGW WenKai **500** display (opsz, negative-tracked) / **400** body; mono uppercase 0.08–0.18em; ≥1.4cqw floor.
...
- **LXGW WenKai (opsz 8..60) + LXGW WenKai via Google Fonts** — the optical-size axis is critical; a non-opsz fallback flattens the size-aware letterforms. CJK: LXGW WenKai (display) / Noto Serif SC (body) / Noto Sans Mono CJK (chrome).
...
本视频可见文字为中文。中文字体统一用 **LXGW WenKai**（霞鹜文楷），文件已就位于 `assets/fonts/`：

- `assets/fonts/LXGWWenKai-Regular.ttf` — weight 400
- `assets/fonts/LXGWWenKai-Bold.ttf` — weight 700

**每个 frame 的 `<template>` 内 `<style>` 必须包含以下 `@font-face`**（`font-family` 用 `LXGW WenKai`，禁止使用系统 CJK 字体名如 `Noto Sans CJK` / `Hiragino Sans`）：

```css
@font-face { font-family: "LXGW WenKai"; font-weight: 400; font-style: normal; font-display: block; src: url("assets/fonts/LXGWWenKai-Regular.ttf") format("truetype"); }
@font-face { font-family: "LXGW WenKai"; font-weight: 700; font-style: normal; font-display: block; src: url("assets/fonts/LXGWWenKai-Bold.ttf") format("truetype"); }
```

- weight 500/600 的标题一律用 `font-weight: 700`（Bold 文件）落地，避免 faux-bold。
- 楷体没有等宽/衬线区分，原 mono 角色（label / chrome）同样用 LXGW WenKai，靠 `letter-spacing` 拉开层级即可。
- 中文字号比英文偏大：可见 body 文字建议 ≥ 2.0cqw，标题按本文件 display ramp；正文用 weight 400，标题/关键词用 700。
